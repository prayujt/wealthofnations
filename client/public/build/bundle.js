...
var app = (function () {
    		...
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = [
    					...
    					listen_dev(button, "click", /*initialize*/ ctx[3], false, false, false)
    				];
    ...
    function create_fragment(ctx) {
    	...
    	function select_block_type(ctx, dirty) {
    		if (!/*gameStarted*/ ctx[0]) return create_if_block;
    		...
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = [
    					...
    					listen_dev(window, "beforeunload", /*close*/ ctx[4], false, false, false)
    				];
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	const initialize = () => {
    		color = "#" + (Math.random() * 16777215 << 0).toString(16).padStart(6, "0");
    	...
    	const handleKeydown = event => {
    		...
    		if (key == "s" && gameStarted) {
    			...
    			database.ref("users/" + username).transaction(user => ({ ...user, x: user.x + pixels }));
    	...
    	database.ref("users").on("value", snapshot => {
    		...
    		$$invalidate(2, users = Object.keys(data ? data : []).map(k => ({ ...data[k], name: k })));
    	});
    	...
    	$$self.$$set = $$props => {
    		if ("database" in $$props) $$invalidate(6, database = $$props.database);
    	};
    ...
    class App extends SvelteComponentDev {
    	constructor(options) {
    		super(options);
    		init(this, options, instance, create_fragment, safe_not_equal, { database: 6 });
    		...
    		if (/*database*/ ctx[6] === undefined && !("database" in props)) {
